...
bool gsm_msg_read(uint16_t index)
{
	...
	if (gsm.msg.textMode == 1) {
		...
		sscanf((char *)gsm.msg.buff,
		       ...
		       gsm.msg.status, gsm.msg.number, &gsm.msg.time.year, &gsm.msg.time.month,
		       &gsm.msg.time.day, &gsm.msg.time.hour, &gsm.msg.time.minute,
		       &gsm.msg.time.second);
		uint8_t cnt = 0;
		...
		while (s != NULL) {
			...
			if (cnt == 6) {
				...
				if (end != NULL) {
					...
					memset(&gsm.msg.buff[end - s], 0,
					       sizeof(gsm.msg.buff) - (end - s));
					return true;
				} else
					return false;
			}
