...
extern k_tid_t gsm_main_th_tid;
...
Gsm_Ftp_Error_t gsm_gprs_ftpUploadBegin(bool asciiFile, bool append, const char *path,
					const char *fileName, const uint8_t *data, uint16_t len);
Gsm_Ftp_Error_t gsm_gprs_ftpUpload(const uint8_t *data, uint16_t len);
bool gsm_gprs_ftpUploadEnd(void);
bool gsm_gprs_ftpExtUploadBegin(bool asciiFile, bool append, const char *path,
				const char *fileName);
